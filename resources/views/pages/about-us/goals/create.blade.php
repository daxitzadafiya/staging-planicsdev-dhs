            ...
            <div class="intro-y box p-5">
                <form action="{{ isset($goal) ? route('goals.update', $goal) : route('goals.store') }}" method="post" enctype="multipart/form-data">
                    ...
                    <div>
                        ...
                        <input id="title" type="text" name="title" class="form-control w-full" value="{{ isset($goal->title) ? $goal->title : old('title') }}" placeholder="Please Enter Title">
                        @error('title')
                    ...
                    <div class="mt-3">
                        ...
                        <textarea name="description" id="description" class="form-control w-full" cols="10" rows="10" placeholder="Please Enter Description">{{ isset($goal->description) ? $goal->description : old('description') }}</textarea>
                        @error('description')
                            <small class="text-danger">{{ $message }}</small>
                        @enderror
                    </div>
